...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (msg == "start2") {
    // Reset semua flag untuk memungkinkan multiple laps
    systemActive = true;
    irDetected = false;
    detectionTime = 0;
    ...
    buzzerBeep(300);
    Serial.println("START diterima. Menunggu deteksi sensor. (Lap baru)");
  }
  else if (msg == "reset") {
    resetSystem();
  }
...
void loop() {
  ...
  if (millis() - lastIRCheck >= irInterval) {
    ...
    if (systemActive && (distance != -1)) {
      if (distance < 1) {  // <1 cm dianggap finish
        irDetected = true;
  ...
  // Auto-reset setelah 10 detik untuk memungkinkan lap berikutnya
  if (irDetected && millis() - detectionTime >= 10000) {
    resetSystem();
    Serial.println("✅ Auto-reset selesai. Siap untuk lap berikutnya.");
  }
}