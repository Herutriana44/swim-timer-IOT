...
def on_message(client, userdata, msg):
    ...
    if message == "start1":
        if not lane.timer_running:
            # Start baru atau start setelah stop (multiple laps)
            if lane.finished:
                # Ini adalah lap baru setelah finish sebelumnya
                # Hitung lap time dari start_time sebelumnya jika ada
                if lane.start_time:
                    # Simpan waktu finish sebelumnya sebagai lap time
                    lane.lap_count += 1
                    lane.last_lap_time = lane.elapsed_time
                    print(f"🏁 Lintasan {lane_num} - Lap {lane.lap_count} selesai: {lane.last_lap_time}")
                
                # Reset untuk lap baru
                lane.finished = False
                lane.start_time = datetime.now()
                lane.elapsed_time = timedelta(0)
                lane.checkpoint_times = {}  # Reset checkpoint times untuk lap baru
                lane.checkpoint_data = {}    # Reset checkpoint data untuk lap baru
                print(f"🏊‍♂️ Lintasan {lane_num} - Lap baru dimulai (Lap {lane.lap_count + 1})")
            else:
                # Start pertama kali
                lane.start_time = datetime.now()
                lane.elapsed_time = timedelta(0)
                ...
                print(f"🏊‍♂️ Lintasan {lane_num} - Timer STARTED (Lap pertama)")
            
            lane.timer_running = True
        else:
            # Lap time (jika timer sudah running - multiple start saat masih berenang)
            if lane.start_time:
                current_time = datetime.now()
                lane.last_lap_time = current_time - lane.start_time
                lane.lap_count += 1
                # Reset start_time untuk lap baru
                lane.start_time = datetime.now()
                lane.checkpoint_times = {}  # Reset checkpoint untuk lap baru
                lane.checkpoint_data = {}
                print(f"🏁 Lintasan {lane_num} - Lap {lane.lap_count} selesai: {lane.last_lap_time}")
                print(f"🏊‍♂️ Lintasan {lane_num} - Lap baru dimulai (Lap {lane.lap_count + 1})")
